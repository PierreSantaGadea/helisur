...
class AeronavesService @Inject constructor
    ( private val api: AeronavesApiClient ) {
        ...
        return withContext(Dispatchers.IO) {
            ...
            when (response.code()) {
                Constants.RESPONSE_CODE._200 ->
                    response.body()!!
                Constants.RESPONSE_CODE._400 ->
                    ObtieneModelosAeronaveCloudResponse(Constants.RESPONSE_CODE.FAILED,null,Constants.ERROR.MESSAGE_400)
                Constants.RESPONSE_CODE._401 ->
                    ObtieneModelosAeronaveCloudResponse(Constants.RESPONSE_CODE.FAILED,null,Constants.ERROR.MESSAGE_401)
                Constants.RESPONSE_CODE._403 ->
                    ObtieneModelosAeronaveCloudResponse(Constants.RESPONSE_CODE.FAILED,null,Constants.ERROR.MESSAGE_403)
                Constants.RESPONSE_CODE._404 ->
                    ObtieneModelosAeronaveCloudResponse(Constants.RESPONSE_CODE.FAILED,null,Constants.ERROR.MESSAGE_404)
                Constants.RESPONSE_CODE._500 ->
                    ObtieneModelosAeronaveCloudResponse(Constants.RESPONSE_CODE.FAILED,null,Constants.ERROR.MESSAGE_500)
                else -> { // Note the block
                    ObtieneModelosAeronaveCloudResponse(Constants.RESPONSE_CODE.FAILED,null,Constants.ERROR.MESSAGE_OTHER)
                }
            }
        }
    }
