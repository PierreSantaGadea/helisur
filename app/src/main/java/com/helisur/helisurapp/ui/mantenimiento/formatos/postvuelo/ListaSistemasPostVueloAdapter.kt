...
class ListaSistemasPostVueloAdapter(
    val ctx: Context, private val mListSistemas: ArrayList<Sistema>
) : RecyclerView.Adapter<ListaSistemasPostVueloAdapter.MyViewHolder>() {
    ...
    ): ListaSistemasPostVueloAdapter.MyViewHolder {
        val vieww =
            ...
            if (holder.rvTareas.isVisible) {
                ...
                if (appItem.tareas != null) {
                    if (appItem.tareas!!.size > 0) {
                        ...
                        val adapter = ListaTareasPostVueloAdapter(ctx,appItem.tareas!!)
